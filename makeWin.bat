...
if "%RELEASE%"=="YES" (
    ...
    if not exist "unwanted_apps.txt" (
        echo FAIL: unwanted_apps.txt not found in project root
        exit /b 1
    )
)
...
conan install . --output-folder="%BUILD_DIR%" --build missing -s build_type=Release -pr "%CONAN_PROFILE%" -c tools.cmake.cmaketoolchain:generator=Ninja
if errorlevel 1 (
...
:: ---- Build QuaZip and other dependencies if needed ----

if not exist "%PREFIX%" (
    echo.
    echo --- Building DEPS in %PREFIX% ---
    echo.
    set "CONAN_BUILD_DIR=%CD%\%BUILD_DIR%"
    call makeWinDeps.bat
    if errorlevel 1 (
        echo.
        echo FAIL: makeWinDeps.bat failed.
        echo.
        exit /b 1
    )
) else (
    echo.
    echo --- DEPS already present in %PREFIX%, skipping ---
    echo.
)
...
copy "%PREFIX%\bin\quazip1-qt6.dll" "%APP_DIR%\" >nul
:: Ninja puts DLLs directly in BUILD_DIR, not a Release\ subdirectory
...
xcopy /s /e /i /q "%APPLICATIONS_DIR%"  "%APP_DIR%\applications\" >nul
...
for /f "usebackq tokens=* eol=#" %%a in ("unwanted_apps.txt") do (
    if not "%%a"=="" (
        echo   Removing: %%a
        if exist "%APP_DIR%\applications\%%a" rmdir /s /q "%APP_DIR%\applications\%%a"
    )
)
